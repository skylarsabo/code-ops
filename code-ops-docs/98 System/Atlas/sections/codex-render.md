...
The rendered vault workflow includes the review-plan handoff required by inventory v2. Host projections must carry both the command sequence and the rewrite-tolerant receipt semantics byte-for-byte from the canonical plugin; otherwise one host can create an adoption another host cannot verify.
