...
Both host projections now carry the Run Contract v3 runtime helpers, local review publisher, judgment planner/scorer, and `local-review-gate` skill from canonical source. Renderer checks prove those scripts and skill instructions remain current; neither host projection owns an alternate review or runtime policy.
