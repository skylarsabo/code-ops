...
Both host projections now carry the Run Contract v3 runtime helpers, shared portable-path, linked-component, regular-index, and hidden-index guards, local review publisher, judgment planner/scorer, and `local-review-gate` skill from canonical source. Renderer checks prove those scripts and skill instructions remain current; neither host projection owns an alternate review or runtime policy.

The Codex projection copies the `SessionEnd` receipt hook and its vendored transcript library byte-identically. The opencode projection carries the two scripts but ports no hook for session end, the same gap the routing card already has there.
