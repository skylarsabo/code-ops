...
Vault standard v4 separates authority from location. Manifest-v2 record collections may preserve immutable evidence at permanent historical paths, while generated inventory, citations, curation, and semantic indexes stay beneath the hub's `98 System/Records/`. A legacy-path exception requires a declared collection so CI can verify its evidence rather than trusting manifest shape. Adoption is irreversible, but current authority can move through append-only curation and a canonical hub document. Manifest v1 remains valid for v3 and collection-free v4 repositories.
...
The adoption review has two time domains. Before adoption, any `HEAD`, canonical Git-index manifest or candidate content, or history change invalidates the plan. Review plans stay at repository-relative ignored paths. The reviewed current path admission anchors record identity. The last matching content commit supplies the citation baseline. After adoption, a content-preserving rewrite may replace locators but cannot increase reviewed risk counts. Checkout-only transformations never change evidence identity; semantic index-to-worktree divergence remains a failure.
