...
The documentation skills now share one v4 authority model. `vault` owns adoption and migration, `repo-docs` owns bounded extraction, `doc-alignment` reconciles current authority, and `atlas` cites preserved evidence by record ID. The code-ops-suite vendors the records engine with its shared library, so every supported host executes the same identity and history rules. The shared boundary rejects symlinked or drive-qualified paths before reads and writes. Canonical stage-0 blobs own content identity and native metadata, while Git's content-aware worktree comparison handles checkout transformations and real local edits separately.

Vault migration must make irreversible judgment durable. The skill plans adoption to a repository-relative ignored receipt and requires explicit dispositions for risky candidates. Adoption consumes that receipt before authored documents move. Protected repository review authenticates the unkeyed receipt checksum. Total-history replacement requires an external trust anchor.
