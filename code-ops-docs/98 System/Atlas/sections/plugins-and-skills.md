...
The traceless scanner is one canonical script shared by the code-ops and privacy packages. Unicode emoji properties keep topology glyphs out of findings. A single-file dash baseline must resolve to separate verified text and subtracts inherited density only; every attribution, tool, phrase, boilerplate, and emoji rule still scans the complete edited text.
