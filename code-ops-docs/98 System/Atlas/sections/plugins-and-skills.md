...
Vault migration must make irreversible judgment durable. The skill plans adoption to an ignored receipt, requires explicit reviewed dispositions for risky immutable candidates, and supplies that receipt to adoption before authored documents move. A bare adoption command is valid only when the history profile needs no operator disposition.
