...
attempted tool calls against an explicit host-agent binding, or the legacy environment/default budget when no binding exists, and on the lead's own dispatch it
only advises.
`session-receipt.mjs`
