...
`CLAUDE.md` and `AGENTS.md` are byte-identical host contracts. They put unenforced safety rules first, then describe repository routing deltas, generated-output boundaries, documentation authority, and required gates. A change to either contract must update the other in the same commit. The traceless rule names the `Traceless publishing (PR commits, title, body)` CI step as its fail-closed backstop and says the tool hook scans published argument values. The contract names each host's traceless hook event and skill-id form, and it points to `INFRASTRUCTURE.md` for the per-host hook table. The writing-standard and code-standard sections each name their hub technique page as the single source and the pin lists that hold the four conventions copies byte-identical.

The routing contract defers general model behavior to the user-wide contract and adds only repository deltas. A version-4 Run Contract needs two parallel work operatives unless `orchestration.singleUnitReason` records why one unit suffices. The dispatch guard denies `general-purpose` and unnamed agent types without a `Wide-surface reason:` line. Above the context ceiling, 300,000 tokens by default, it denies new dispatches until a handoff assessment runs. The contract names the wide-type deny, the context-ceiling gate, and the round stop as enforcement, and the routing card, dispatch ledger, and narration scan as advisories.
