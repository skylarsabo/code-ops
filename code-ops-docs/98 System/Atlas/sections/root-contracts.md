...
`CLAUDE.md` and `AGENTS.md` are byte-identical host contracts. They put unenforced safety rules first, then describe model routing, acceptance ownership, generated-output boundaries, documentation authority, and required gates. A change to either contract must update the other in the same commit. The traceless rule names the `Traceless publishing (PR commits, title, body)` CI step as its fail-closed backstop and says the tool hook scans published argument values. The writing-standard and code-standard sections each name their hub technique page as the single source and the pin lists that keep the four conventions copies identical.
