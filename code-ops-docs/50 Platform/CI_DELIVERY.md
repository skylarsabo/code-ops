...
The record-collection eval exercises immutable evidence contracts on Ubuntu on every change, and on Windows when the range under test touched the record scripts, their eval, the generated record authority, or the workflow lines that invoke the record eval, so an unrelated eval step added to the workflow does not pay for the Windows run; the documentation manifest is not on the list because it re-syncs on nearly every change; the weekly scheduled run runs it on Windows unconditionally, and a manual dispatch does not, so an on-demand proof run for the macOS host evals costs no Windows record run. That gate removes about twelve minutes from an unrelated change's Windows leg. Repositories with record collections require a complete checkout using `fetch-depth: 0` and `filter: ""`. A shallow or partial checkout is infrastructure failure, not evidence loss.
