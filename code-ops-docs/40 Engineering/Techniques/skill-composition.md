...
Most skills in this marketplace are self-contained. Each one reads its `CONVENTIONS.md`,
runs its loop, and produces an artifact.

A smaller set also invokes a named skill mid-run, either in another plugin or in its own.
An invocation is a fallback producer when an expected register is missing, a baseline pass
before the main work, or a specialist audit gated on what the diff touches. A larger set
names another skill without calling it. Such a reference is a hand-off target for work
that leaves the current skill's mandate, or a routing pointer to a better-fitting neighbor.

This page is the ground truth for both kinds. The table under **The edges** carries one row
for every qualified `<plugin>:<skill>` reference found in `plugins/*/skills/*/SKILL.md`. A
reference counts with or without its leading slash, and each skill's own name is excluded.
A skill's self-declaring "Invoked as" line needs no rule of its own, because the only
reference it carries is that same self name. The **When** column says which kind of edge
each row is.

`scripts/lint-plugins.mjs` derives the same set from the rows under that heading. It fails
closed when a reference has no row, and when a row has no reference. An edge-shaped row
anywhere else on the page also fails closed, so parking a row outside the heading cannot
bypass the derivation. Fenced code blocks are skipped on both sides, because a Markdown
example on this page is illustration and not page structure.
...
## Script references and the entrypoint façade

The edges above are skill-to-skill. A skill also references the scripts it runs, and
`lint-plugins.mjs` gates those references the same way. Every
`${CLAUDE_PLUGIN_ROOT}/scripts/X` a skill names must be bundled in that plugin and stay
byte-identical to the canonical `scripts/X`.

A skill may reach a script through the entrypoint façade instead, written as
`co.mjs <domain> <verb>`. The linter resolves the verb through the table inside `co.mjs` to
the sibling script it runs, so the façade carries the same bundling requirement as the
direct path. A verb the table does not carry fails closed. The direct paths still work.

## The handoff contract

An invocation only works if both sides agree on the artifact's shape. That contract lives
in each plugin's `CONVENTIONS.md`, sections 6 through 9, which define the schemas, the
evidence tiers, and the hand-off map. The standard filenames above (`GROUND_TRUTH.md`,
`FINDINGS_REGISTER.md`) are never ad hoc per skill.
