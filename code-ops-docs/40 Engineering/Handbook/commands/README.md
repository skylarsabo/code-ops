...
| **Audit a risky subsystem and trust the result** (depth + proof) | `/rigor:ground-truth` → `/rigor:test-suite-audit` → `/rigor:bug-hunt` + `/rigor:quality-scan` → `/rigor:safety-net` → `/rigor:fix-verified` | rigor | The verification journey. See [audit-a-risky-subsystem](../../../70 Guides/Guides/audit-a-risky-subsystem.md). |
