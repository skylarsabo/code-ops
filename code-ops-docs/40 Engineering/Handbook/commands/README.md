# Command Reference: Index and Task Router

This page routes a goal to a command. It says how to read a command entry, maps the goal
you actually have to the commands that serve it in the right order, and points at the
...
| --- | --- |
| **Invoke** | The exact workflow name. Claude Code uses `/code-ops-suite:codebase-audit`, and Codex uses `code-ops-suite:codebase-audit`. |
| **One-liner** | The single sentence from the plugin's own `README.md` Skills list. |
| **Mode** | The operating mode the skill declares (AUDIT, IMPLEMENT/writes code, DISCOVERY, REVIEW, DOCUMENT). DISCOVERY, REVIEW, and DOCUMENT never edit source. |
...
| **Checkpoints** | Where it pauses for you. Every command is developer-in-the-loop, and orchestrators pause at each phase boundary. |
...
That shape mirrors the generated host-specific `SKILL.md` header each command carries,
which is a `description` and a `Mode · Produces` line. The description's "Use when…"
clause is what the model reads to decide whether to route to the skill. The routing
discipline itself lives in that description, the session routing card, and each skill's
own checkpoints. What you read in the reference is what the skill declares about itself,
never a paraphrase.

A few conventions hold for every command in every plugin. They are the shared backbone,
described in [the mental model](../02-mental-model.md):

- **Developer-in-the-loop.** Nothing irreversible happens without your say-so at a checkpoint.
- **Evidence at `file:line`.** Findings cite their location, and claims name a source and a tier.
- **Behavior-preserving.** Changes preserve observable behavior unless you approve otherwise.
- **Registers are the single source of truth**, stamped `Verified-at <sha>`. The bundled `revalidate-register.mjs` re-checks freshness against the current tree before any hand-off.
- **The automation ladder** runs gated, auto-safe, or auto-all, with always-gated categories: security and authentication, secrets, data migrations, public contracts, and destructive operations. See [choosing an automation level](../../Techniques/choosing-an-automation-level.md).
...
the order to run them. `→` means "then", `+` means "in parallel, both", and
`(orchestrator)` marks a single command that strings the others together for you.
...
- **Requires `<plugin>`** means the route does not run without it. The skill declares the prerequisite and stops.
- **Optional: `<plugin>`** means the route runs without it, and the cross-plugin step is skipped or replaced by a fallback the skill names.
...
| **Run the whole anonymity track** | `/code-ops-suite:everything plugins: privacy` (orchestrator) | privacy-opsec-suite | model → audits → harden → docs/gate, pausing at each phase boundary. |
...
| **Run the rigor pipeline end-to-end** | `/code-ops-suite:everything plugins: rigor` (orchestrator) | rigor | Start with `assess-only` to get proven findings before changing anything. |
...
> → privacy gates.** When in doubt, start broad with code-ops-suite. Reach for `rigor`
> when you need proof, `privacy-opsec-suite` when the project has anonymity needs, and
...
The rows above name skills. These rows name bundled command-line tools, reached through
the `co` entrypoint that `code-ops-suite` vendors. They are not skills, so they take no
checkpoint and write nothing you did not ask for. Run them directly, or let a skill run
them for you.

| I want to… | Run | Notes |
| --- | --- | --- |
| **Skim a file before reading it** | `co context skim <file>` then `co context skim <file> --range A,B` | Prints an outline: headings with section spans, code definitions with import and export rows, top-level JSON keys. Never prints bodies. |
| **Answer a structural question without reading a map** | `co context query find\|callers\|callees\|blast <symbol>` | Answers with `file:line` anchors and edge lists from a home-directory symbol index. `co context query status` reports the index and its providers. |
| **Compress a command's output before it enters the context** | `co context digest -- <cmd>` | Shape-keyed compression with a must-keep contract. The child's exit code becomes the digest's, and every elided region prints the `sed` that recovers it. |
| **Check a diff for over-building** | `co scan overbuild --git <range>` | Eight deterministic tells against the code-economy ladder. Advisory on every tell except an unrecorded dependency, which exits 1. |
| **Find every deliberate simplification** | `co scan deferrals` | Collects `deferred(<ceiling>, <upgrade path>)` markers into `DEFERRALS_REGISTER.md`, in the grammar `revalidate-register.mjs` re-greps. `--check` reports drift. |
...
see [chapter 03](../03-orchestrators.md).
