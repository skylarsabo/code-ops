...
- [../Techniques/code-standard.md](../Techniques/code-standard.md): the house code standard every change is built to: design first at a size proportionate to the change, the least correct and readable code, earned modularity, measured optimization, and reason-only comments, with each rule's mechanical backstop named.
