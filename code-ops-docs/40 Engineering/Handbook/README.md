...
- [../Techniques/code-standard.md](../Techniques/code-standard.md): the house code standard every change is built to: design first at a size proportionate to the change, the smallest correct, readable solution, earned modularity, measured optimization, reason-only comments, proportionate verification, and no repeated work, with each rule's mechanical backstop named.
