...
Canonical package metadata is JSON in `plugins/*/.claude-plugin/plugin.json` and `.claude-plugin/marketplace.json`. Generated marketplace metadata is a projection of that source. Evidence: `AGENTS.md:95-104`.
...
A session receipt is one JSON line, version `1`, with `ts`, `sessionId`, `cwd`, `reason`, `durationMs`, `models`, `turns`, `toolCalls`, `toolResultChars`, `contextAtEnd` (the tokens the last assistant message carried in), `arms` (`digest`, `ladderCard`, `index`, and `handoffCard`, each a boolean read from the same switch the hook reads), `handoff` (`band`, the highest band the session's handoff marker reached, and `invoked`, true when the transcript shows a `/code-ops-suite:handoff` call), `files`, `skipped`, and `tokens` split into `main` and `subagents`, each with `input`, `cacheRead`, `cacheCreate`, `output`, `thinking`, and `total`. Usage is deduplicated by message id because the host writes one assistant message as several transcript lines that repeat the same usage block. A new field is added without a version bump, because every reader tolerates an unknown key and treats a missing one as absent. Evidence: `plugins/code-ops-suite/hooks/session-receipt.mjs:62-90` and `scripts/transcript-lib.mjs:141-156`.
