...
`context-audit.mjs receipts` reads the ledger back and accepts only version `1` rows.

The `PreCompact` hook `precompact-preserve.mjs` emits one fixed `additionalContext` instruction naming the six items a compaction summary must keep. It reads no stdin, adds no per-turn tokens, and exits `0` on every path. Evidence: `plugins/code-ops-suite/hooks/precompact-preserve.mjs:13-32`. Evidence: `scripts/context-audit.mjs:77-90`.
