...
Inventory v2 stores `adoptionReview` beside records and artifacts. Its `receiptDigest` covers source bindings and reviewed candidates. Pre-adoption plans bind exact history. Post-adoption checks require exact history while the review anchor remains ancestral; after a content-preserving rewrite replaces that anchor, current bytes must still match and recomputed risk counts cannot exceed the reviewed counts. Commit rewrites may change locator fields without invalidating authority. Adopted record entries store `introducedCommit` for exact-path provenance and `baselineCommit` for citation resolution. Inventory v1 remains readable without these fields.
