...
`classify` reports partition validity and historical adoption readiness. Invalid classification reports `classification-invalid` even when history is unavailable. Uncommitted index candidates report `pending-commit` without invalidating their structural classification. `plan-adoption` writes only to an ignored repository path. The plan binds `HEAD`, manifest bytes, candidate bytes, and path history. `adopt --review` recomputes those bindings. Historically revised immutable candidates require a `freeze-current` disposition and rationale.

Inventory v2 stores `adoptionReview` beside records and artifacts. Its `receiptDigest` covers source bindings and reviewed candidates. Pre-adoption plans bind exact history. With complete history, post-adoption checks require exact current bytes and classification, consistent stored risk labels, current-risk rationale coverage, non-increasing risk counts, and exact original-candidate coverage. Incomplete history warns during ordinary checks and fails strict verification as infrastructure. Commit rewrites may change locator fields without invalidating authority, and `sourceHead` never selects a verification mode. The unkeyed digest is not a reviewer signature; protected repository review supplies the procedural trust root. Total-history replacement needs an external signature or transparency log to authenticate the original receipt bytes. Adopted record entries store `introducedCommit` for exact-path provenance and `baselineCommit` for citation resolution. Inventory v1 remains readable without these fields.
