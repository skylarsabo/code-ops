...
- **Local first:** installed-dependency capabilities/APIs via `<plugin-root>/scripts/lib-docs.mjs` (or the `code-ops-docs` `get-docs` MCP when `code-ops-suite` is installed) — the *installed* version, no query egress (`§2`). Triangulate against the library's own types and our usage.
