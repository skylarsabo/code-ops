...
**Invoke in Codex by naming `researcher:research-spike`.** Read §A, §2, §3, §4, §6, §7, §8, §10, §11, §12, §13, and §14 of
...
- **Local first.** Dispatch gatherers, in parallel over the candidate approaches, over the
  ...
  `<plugin-root>/scripts/lib-docs.mjs`, or through the `code-ops-docs` MCP server's
...
> `node <plugin-root>/scripts/research-manifest.mjs validate <brief>`, which fails
