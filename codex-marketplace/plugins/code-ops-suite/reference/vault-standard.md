...
Each switch is `CODE_OPS_INDEX`, `CODE_OPS_DIGEST`, `CODE_OPS_LADDER_CARD`, or `CODE_OPS_RECEIPTS`, and each holds `off`, `0`, or `false` in the host environment. A repository documents its own switches in its platform pages rather than here.
