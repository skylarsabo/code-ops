...
```
<repo>-docs/98 System/Atlas/MANIFEST.json      machine index: the ONLY place stamps and scopes live
<repo>-docs/98 System/Atlas/INBOX.md           append-only dated observations, consolidated periodically
<repo>-docs/98 System/Atlas/sections/<slug>.md judgment prose, one file per section
```

`<repo>-docs/` is the repository's documentation hub, as the vault standard defines it. In this
repository that is `code-ops-docs/`. Fallback: `<repo-root>/atlas/` when the repo has no
`<repo>-docs/` hub. The atlas lives in
