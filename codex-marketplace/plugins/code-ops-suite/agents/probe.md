...
> Codex role contract: this file is a briefing template for a collaboration subagent. Before dispatch, the lead reads `agents/model-floors.json` and routes `probe` at or above its `mid` floor. This role is read-only: return the report inline.
