...
**Invoke in Codex by naming `code-ops-suite:debug`.** First read the `<plugin-root>/CONVENTIONS.md` bundled with this plugin — operating model, interaction protocol, safety rails, quality lenses, and the implementation loop this skill follows.
...
Run `node <plugin-root>/scripts/preflight.mjs --artifact-dir <run folder>` — a FAIL stops the run before any fan-out; advisories are noted in the register. Capture the symptom precisely; run `rigor:ground-truth` for the baseline. Use `rigor`'s verifier to build a **reliable reproduction** (a failing test or runnable repro). If it can't be reproduced, stop and report exactly what's needed (env, data, steps) — never guess a fix.
