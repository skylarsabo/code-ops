...
**Invoke in Codex by naming `code-ops-suite:current-docs`.** First read the `<plugin-root>/CONVENTIONS.md` bundled with this plugin — this skill implements its "documentation/reference lookup" capability (`§2`) in-house: local-first, no third-party indexer, no query egress.
