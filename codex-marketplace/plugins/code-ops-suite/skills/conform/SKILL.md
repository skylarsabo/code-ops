...
**Invoke in Codex by naming `code-ops-suite:conform`.** First read the `<plugin-root>/CONVENTIONS.md` bundled with this plugin — the operating model (`§1`), the interaction protocol (`§3`), the safety rails (`§4`), the evidence tiers (`§7`), the single-source-of-truth conventions (`§12`) that decide where this run's artifacts land, and the doc standard (`§13`).
...
**Consent decides who is in.** The manifest names a repo and the repo consents, by carrying the phrase `fleet member: yes` in a `## Fleet` section of its own standards contract. Three rules follow, and none of them bends:
