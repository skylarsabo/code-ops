...
**Invoke in Codex by naming `code-ops-suite:vault`.** First read the `<plugin-root>/CONVENTIONS.md` bundled with this plugin — the interaction protocol (`§3`), the safety rails (`§4`), the single-source-of-truth conventions (`§12`) that decide where a run's artifacts land, and the doc standard (`§13`) — and `docs/techniques/vault-standard.md`, which is the source of truth for the layout, the profiles, and the promotion rule.
