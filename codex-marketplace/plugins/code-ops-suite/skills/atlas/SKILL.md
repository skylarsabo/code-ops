...
**The tool owns the stamps.** `<plugin-root>/scripts/atlas-check.mjs stamp` is the only sanctioned writer of `verifiedAt`; hand-editing `MANIFEST.json` is how a section that has gone stale keeps claiming to be current.
