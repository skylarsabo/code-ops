...
**The tool owns the manifest.** `<plugin-root>/scripts/atlas-check.mjs` writes every part of it. `add` registers a section. `stamp` is the only sanctioned writer of `verifiedAt`, `verifiedDigest`, and the `claims` it derives from a section's `path:line` citations. Never edit `MANIFEST.json` by hand.
