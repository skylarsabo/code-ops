...
Fill the sanitized-note template (`docs/techniques/calibration-protocol.md`) from the extracted metrics: counts and deltas against the prior `evals/CALIBRATION_TABLE.md` row, lessons learned — zero paths, code, or URLs from the target. Gate it with `node <plugin-root>/scripts/calibration-metrics.mjs --validate-note <note file>` — a structural scrub that fails closed on any path/code-fence/URL/email; fix the note and re-validate rather than working around a hit.
