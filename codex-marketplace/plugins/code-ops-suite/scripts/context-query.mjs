...
const USAGE = 'usage: context-query.mjs <refresh|status|find|callers|callees|blast|explore> [args] [--root <dir>] [--exclude <prefix>]... [--json] [--fuzzy] [--depth <n>] [--budget <bytes>] [--with-source] [--no-stale-check]';
const INDEX_VERSION = 1;
const MAX_FILE_BYTES = 512 * 1024;

const argv = process.argv.slice(2);
const o = { root: null, json: false, fuzzy: false, depth: 2, budget: 4000, withSource: false, staleCheck: true, excludes: [] };
const positional = [];
...
function indexFile(file, exists) {
  ...
  const defs = definitions(text, ext);
...
function refresh(only = null) {
  ...
  for (const file of targets) {
    ...
    if (files[file]?.sha === sha) { reused++; continue; }
    const entry = indexFile(file, exists);
    if (entry) { files[file] = entry; parsed++; }
  }
  const idx = { version: INDEX_VERSION, root, excludes, builtAt: new Date().toISOString(), head: git(root, ['rev-parse', 'HEAD']).trim(), files };
  saveIndex(idx);
...
switch (command) {
  case 'refresh': {
    ...
    printer([`ok index ${indexPath}: ${Object.keys(r.idx.files).length} files, ${r.parsed} parsed, ${r.reused} unchanged, ${r.dropped} dropped${r.idx.excludes.length ? `, excluding ${r.idx.excludes.join(' ')}` : ''}`],
      { index: indexPath, files: Object.keys(r.idx.files).length, parsed: r.parsed, reused: r.reused, dropped: r.dropped, excludes: r.idx.excludes });
    break;
  }
  case 'status': {
    const idx = loadIndex();
    ...
    printer([`index ${indexPath}`, `built ${idx.builtAt} at ${idx.head.slice(0, 7)}: ${Object.keys(idx.files).length} files, ${defs} definitions, ${edges} call sites`,
      ...
      { index: indexPath, builtAt: idx.builtAt, head: idx.head, files: Object.keys(idx.files).length, definitions: defs, calls: edges, stale });
    break;
  }
  case 'find': {
