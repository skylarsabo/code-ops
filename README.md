...
`opencode-dist/opencode.json` binds every agent to a free OpenCode Zen model meeting its capability tier and leaves the lead unset, so the lead inherits the session model and the install costs nothing. A ready-made config for each supported provider ships under `opencode-dist/configs/`, covering Anthropic, xAI, OpenAI, Google, Z.AI, Moonshot, DeepSeek, Mistral, and GitHub Copilot. Moving the suite between providers is a config swap, not a rewrite. Merge one into your own config rather than overwriting it. The full tier table lives in `opencode-dist/MODEL_TIERS.md`.

GitHub Copilot bills AI Credits from token counts, at 100 credits per dollar. Copy `opencode-dist/configs/model-profile.github-copilot.json` to `~/.claude/code-ops/opencode-model-profile.json`. The lifecycle plugin then binds each tier to the cheapest priced model that meets it: `gpt-6-luna` for light, `gemini-3.8-flash` for mid, and `gpt-6-sol` for strong and frontier. The lead model sets most of the bill, because every turn re-reads its context. An Opus 5.5 or Sol lead reads cached context at $0.20 per million tokens. A Grok 4.7 lead pays $0.50, and every Grok rate doubles above 200,000 tokens. The profile budgets 15,000 credits a month. Run `node ~/.config/opencode/code-ops/cost-report.mjs --check` to apply its cost gates. The command exits 1 when the month projects past the budget, a session peaks past 200,000 tokens, or the lead cache hit rate falls below 80%.
...
- **`code-ops-suite`** covers general engineering for any codebase: audit, security and privacy threat assessment, remediation, feature discovery and build, performance, tests, dependencies, PR review, local review gates, repo docs, standards adoption, onboarding, normalization and concept closure, PR splitting, ship, debug, current docs, architecture and API and data-model and ADR and ops documentation, run handoff, the atlas, the docs vault, the conformance pass, and the suite's own calibration, cost, and parity audits. (31 skills)
...
- **`rigor`** covers verification-first quality: find real bugs and prove each with a repro, validate the test suite with flaky and mutation testing, lock behavior with characterization safety nets, fix at root cause with a regression guard, and ship measured improvements. Prove it or do not report it. (10 skills)
...
└── plugins/
    ├── code-ops-suite/                   # 31 skills, explorer + reviewer + implementer, hooks/
