...
- **`code-ops-suite`** — general engineering for any codebase: audit, security/privacy threat assessment, remediation, feature discovery & build, performance, tests, dependencies, PR review, doc alignment, standards-contract adoption (repo and global), onboarding, code normalization, PR-splitting, ship, debug, current-docs, plus architecture & API/data-model/ADR/ops doc generation, run handoff/resume and the per-repo atlas, plus calibration/cost/provider-parity self-audits. (30 skills)
