...
- `evals/` — the eval harness. Six automated regression evals run in `.github/workflows/validate.yml` (register-staleness, ai-tells, lib-docs engine, MCP smoke, researcher egress-manifest, script-guards) plus a zero-dependency guard and a fixture-drift guard; see `evals/README.md` for these and the judgment-eval approach.
