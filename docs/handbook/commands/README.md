...
The code-ops marketplace ships **62 commands** across four plugins. Call any command as
...
- [code-ops-suite.md](code-ops-suite.md) — **32 commands**: the engineering spine (assess, build, deep-dives, gate/consistency, docs/knowledge, the documentation generators, suite self-audit, and the orchestrators `full-sweep` / `everything` / `ship` / `debug`).
