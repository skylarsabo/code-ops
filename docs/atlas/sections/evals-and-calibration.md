...
- The two hand-written Machine-block grammars (`calibration-metrics.mjs` and `calibration-graph.mjs`) are cross-checked by the calibration-graph eval, so a grammar tweak must land in both. The `atlas:` line (sections/fresh/refreshed/falsified, added 2026-07-30) is the one *optional* metric line, and the run-doc `atlas` field is omitted-when-absent by design — "no atlas leg" and "an atlas nobody used" are different runs; keep any new optional line to that same pattern rather than storing zeros.
- Run↔lesson semantics: a run doc's `lessons` array is the only source of surfaced-in/recurred-in; edges carry only fixed-in/enforced-by/verified-in/deferred/supersedes; a `verified-in` run deliberately does not list the lesson. `fixed-in` accepts `PR-NN` or `COMMIT:<sha>` (both shape-only historical records — neither is resolved externally). Status precedence: SUPERSEDED > ENFORCED (fixed+enforced) > UNENFORCED (fixed-only) > DEFERRED > OPEN.
