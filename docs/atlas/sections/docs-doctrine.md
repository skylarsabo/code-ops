...
- ADRs record decision-time facts and rot on enforcement details: ADR 0001 claimed `check-doc-citations.mjs` was run-on-demand long after it became a CI step (fixed 2026-07-30). When a script gets promoted into a gate, sweep the ADRs that describe its old status — nothing mechanical does.
