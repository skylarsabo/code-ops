...
- The renderer emits `allow_implicit_invocation: true` and the eval pins it; two user-facing doc lines (root README, getting-started) still claim manual-only policy — code + eval agree against the docs (stale docs, pending owner confirmation).
