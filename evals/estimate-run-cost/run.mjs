...
try {
  ...
  check('n. it is named in its own caveat instead',
    /CAVEAT — 1 run folder\(s\) carry a ledger with no parseable dispatch rows \(an\s+aborted or not-yet-started run, or rows the grammar cannot read\)/.test(n.stdout)
