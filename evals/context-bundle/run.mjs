...
try {
  mkdirSync(join(root, 'src'), { recursive: true }); mkdirSync(join(root, 'other')); mkdirSync(runDir);
  ...
  execFileSync('git', ['init', '-q'], { cwd: root }); execFileSync('git', ['config', 'user.email', 'eval@example.test'], { cwd: root }); execFileSync('git', ['config', 'user.name', 'Eval'], { cwd: root }); execFileSync('git', ['add', '.'], { cwd: root }); execFileSync('git', ['commit', '-qm', 'initial'], { cwd: root });
  const snapshotPath = join(runDir, 'CONTEXT_SNAPSHOT.json'); let result = run(snapshotScript, ['prepare', '--root', root, '--out', snapshotPath, '--cache', cache]);
