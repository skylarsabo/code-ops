...
try {
  ...
  const orchestrated = writeContract(1, git(root, ['rev-parse', 'HEAD']), firstSnapshot.snapshotId);
  orchestrated.version = 4;
  orchestrated.budget = { maxDispatches: 3, maxParallel: 2, maxRetriesPerUnit: 1 };
  orchestrated.orchestration = { mode: 'lead-and-operatives', minOperatives: 2, minParallel: 2 };
  orchestrated.units[0].validates = [];
  orchestrated.units[0].independentOf = [];
  orchestrated.units.push({
    ...orchestrated.units[0], id: 'D-002', lens: 'runtime-boundaries', role: 'tracer',
    brief: 'trace runtime boundaries', scope: ['source.txt'], artifact: 'run/TRACE.md',
  });
  orchestrated.units.push({
    ...orchestrated.units[0], id: 'D-003', phase: 'review', wave: 2, lens: 'independent-review',
    role: 'reviewer', kind: 'review', effort: 'high', brief: 'review runtime evidence',
    artifact: 'run/REVIEW.md', dependsOn: ['D-001', 'D-002'], validates: ['D-001', 'D-002'],
  ...
  r = run(CONTRACT, ['check', '--contract', contractPath, '--root', root], root);
  check('version 4 permits independent actors to share a role and model', r.status === 0, r.out);
  const sharedValidatorRole = structuredClone(orchestrated);
  sharedValidatorRole.units[2].role = sharedValidatorRole.units[0].role;
  writeFileSync(contractPath, `${JSON.stringify(sharedValidatorRole, null, 2)}\n`);
  r = run(CONTRACT, ['check', '--contract', contractPath, '--root', root], root);
  check('version 4 permits a validator to share a declared role with validated work', r.status === 0, r.out);
  const validatorHeavy = structuredClone(orchestrated);
  validatorHeavy.units[1].kind = 'review';
  validatorHeavy.units[1].effort = 'high';
  writeFileSync(contractPath, `${JSON.stringify(validatorHeavy, null, 2)}\n`);
  r = run(CONTRACT, ['check', '--contract', contractPath, '--root', root], root);
  check('version 4 minimum counts work operatives, not validators', r.status === 1 && /planned work operatives/.test(r.out), r.out);
  const validatorParallel = structuredClone(orchestrated);
  validatorParallel.units[1].wave = 2;
  writeFileSync(contractPath, `${JSON.stringify(validatorParallel, null, 2)}\n`);
  r = run(CONTRACT, ['check', '--contract', contractPath, '--root', root], root);
  check('version 4 parallel minimum counts work operatives, not validators', r.status === 1 && /work-operative plan/.test(r.out), r.out);
  const unvalidated = structuredClone(orchestrated);
  unvalidated.units[2].validates = [];
  unvalidated.units[2].independentOf = [];
  writeFileSync(contractPath, `${JSON.stringify(unvalidated, null, 2)}\n`);
  r = run(CONTRACT, ['check', '--contract', contractPath, '--root', root], root);
  check('version 4 requires independent validation of every operative unit', r.status === 1 && /D-001 lacks an independent/.test(r.out) && /D-002 lacks an independent/.test(r.out), r.out);
  writeFileSync(contractPath, `${JSON.stringify(orchestrated, null, 2)}\n`);
  const v4Ledger = join(runDir, 'V4_DISPATCH_LEDGER.md');
  const v4Acceptance = join(runDir, 'V4_ACCEPTANCE_LEDGER.md');
  const v4Result = join(runDir, 'V4_RESULT.json');
  writeFileSync(v4Ledger, '| id | role | brief | expected artifact | status |\n| --- | --- | --- | --- | --- |\n| D-001 | gatherer@gpt-5.6-terra | map runtime state | run/REPORT.md | reported |\n| D-002 | tracer@gpt-5.6-terra | trace runtime boundaries | run/TRACE.md | reported |\n| D-003 | reviewer@gpt-5.6-terra | review runtime evidence | run/REVIEW.md | reported |\n');
  writeFileSync(v4Acceptance, `${ACCEPT_HEADER}| Q-001 | 1 | PASS | v4 fixture | tool | |\n`);
  writeFileSync(join(runDir, 'REPORT.md'), '# Report\n');
  writeFileSync(join(runDir, 'TRACE.md'), '# Trace\n');
  ...
  addLegacyV4('reviewer', 'review runtime evidence', 'run/REVIEW.md'); reportV4('D-003');
  r = run(CONTRACT, ['finalize', '--contract', contractPath, '--acceptance', v4Acceptance, '--dispatch-ledger', v4Ledger, '--result', v4Result, '--root', root], root);
