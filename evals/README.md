...
All eighteen are wired into `.github/workflows/validate.yml`.
...
- **`import-graph/`** — pins `scripts/import-graph.mjs`: blank/missing flag values, an unknown flag, and a non-git `--root` all fail closed, a resolved relative import chain (a -> b -> c) reports both `imports` and `imported-by` edges, a directory specifier resolves to its `index` file, an unresolved relative import is listed rather than dropped, a non-literal dynamic `import()` is noted as skipped, a bare specifier is counted but never turned into an edge, an a<->b cycle resolves on both sides without hanging (forward-edge extraction only — no graph traversal), `--focus <path>` restricts the body to that file's one-hop subgraph, and the footer counts match the fixture exactly. Run: `node evals/import-graph/run.mjs`.
- **`lint-plugins/`** — pins `scripts/lint-plugins.mjs`, the repo's main structural gate: it fails closed on version-parity mismatches, a missing "## Done when" section, a wrong handbook router command count, and vendored-script drift, while an orphan root script with no `evals/` reference stays advisory-only and never gates the run. Run: `node evals/lint-plugins/run.mjs`.
- **`release-tooling/`** — pins the plugin release-tooling scripts: `sync-vendored.mjs` drift detection and write-mode sync, `vendored-manifest.mjs` manifest integrity (every entry shaped correctly, every named root script and plugin copy present), `bump-plugin-version.mjs` semver bump semantics (minor/explicit bumps, bad specs, unknown plugins, and version-skew refusal that leaves every file untouched), and `check-plugin-bump.mjs`'s PR version-bump gate (unbumped edits fail, bump-plus-changelog passes, an unresolvable `--base` skips fail-open, and derived `codex-marketplace/` changes are ignored). Run: `node evals/release-tooling/run.mjs`.
- **`doc-citations/`** — pins `scripts/check-doc-citations.mjs`: valid single-line and range citations pass, an out-of-bounds line number and a missing cited file fail closed naming the doc line and reason, a citation inside a fenced code block is ignored, and an inverted range fails closed. Run: `node evals/doc-citations/run.mjs`.
...
- **`scan-narration/`** — pins `scripts/scan-narration.mjs`, the run-artifact narration scanner: stays quiet on a compact summary, flags an over-length report at both the advisory and hard bounds, fails closed on process-narration, reports table-restatement and filler as advisories only, and never trips `PROCESS-NARRATION` on the "I/O throughput" false-positive guard case. Run: `node evals/scan-narration/run.mjs`.
