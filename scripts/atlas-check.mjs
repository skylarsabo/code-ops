...
// Cues that mark a sentence as comparing two code sites. Deliberately small and literal: a wider
// list would flag ordinary prose, and this advisory earns its place only while it stays quiet.
// `byte-identical` is a parity statement about whole files, not a claim about two code sites, so
// a hyphen before `identical` is excluded; `matches` is too common in ordinary prose to keep.
const COMPARISON_CUE_RE = /\b(?:same as|(?<!-)identical|differs?|unlike|disagrees?|mirrors)\b/i;
// How much of a flagged sentence the advisory echoes back.
const MAX_ADVISORY_ECHO = 80;

// Sentences that compare two code sites while citing fewer than two of them. Advisory only: it
// changes no verdict and no exit code. WHY: a claim is derived from a citation, so a comparison
// carrying one citation registers as one claim, and the uncited half can drift with nothing to
// notice. Citing both sides makes the pair two claims, and either half drifting gates the
// section. Fenced blocks and headings are skipped, because quoted output and a title are not
// prose making a claim.
function uncitedComparisons(atlasDir, section) {
  let prose;
  try { prose = readFileSync(resolve(atlasDir, section.file), 'utf8'); } catch { return []; }
  let fenced = false;
  const kept = [];
  for (const line of prose.split(/\r?\n/)) {
    if (/^\s*(?:```|~~~)/.test(line)) { fenced = !fenced; continue; }
    kept.push(fenced || /^\s*#/.test(line) ? '' : line.trim());
  }
  const out = [];
  // A blank line ends a paragraph, so a sentence never runs across one. Within a paragraph the
  // line wrapping is arbitrary, so join before splitting on sentence ends.
  for (const para of kept.join('\n').split(/\n\s*\n/)) {
    for (const sentence of para.replace(/\n/g, ' ').split(/(?<=[.!?])\s+/)) {
      if (!COMPARISON_CUE_RE.test(sentence)) continue;
      const cited = new Set();
      for (const m of sentence.matchAll(CLAIM_REF_RE)) cited.add(`${m[1]}:${m[2]}`);
      if (cited.size < 2) out.push(sentence.trim().slice(0, MAX_ADVISORY_ECHO));
    }
  }
  return out;
}
...
// Rewrites the manifest in the style the file already carries. A target repo's formatter or
// linter may own JSON style, and an assess-only run must not leave a lint error in a file it
// created. Detects the existing indent (two spaces, four spaces, or a tab) and end-of-line
// (LF or CRLF); with no existing file it writes two spaces and LF. Key order is never touched:
// the manifest's meaning lives in the keys, not in their layout.
function serializeManifest(existingText, manifest) {
  let indent = 2;
  let crlf = false;
  if (typeof existingText === 'string' && existingText !== '') {
    crlf = existingText.includes('\r\n');
    // The first indented line of a JSON object sits exactly one level deep, so its leading run
    // is the file's indent unit.
    const lead = existingText.match(/^[ \t]+(?=\S)/m);
    if (lead) indent = lead[0].startsWith('\t') ? '\t' : (lead[0].length === 4 ? 4 : 2);
  }
  const body = JSON.stringify(manifest, null, indent) + '\n';
  return crlf ? body.replace(/\r?\n/g, '\r\n') : body;
}

// Loads and schema-validates MANIFEST.json. Returns { manifest, raw, violations } — violations is
// a list of human-readable schema failures; a non-empty list is always fatal for the caller
// (fail closed), never a downgraded advisory. `raw` is the file's own text, which
// `serializeManifest` reads a rewrite's style from.
function loadManifest(atlasDir) {
  ...
  catch (e) { return { path, manifest: null, raw, violations: [`MANIFEST.json is not valid JSON: ${e.message}`] }; }
  if (manifest === null || typeof manifest !== 'object' || Array.isArray(manifest))
    return { path, manifest: null, raw, violations: ['MANIFEST.json must be a JSON object'] };
  ...
  if (!Array.isArray(manifest.sections)) {
    ...
    return { path, manifest, raw, violations };
  }
...
function cmdInit(args) {
  ...
  try {
    ...
    writeFileSync(manifestPath, serializeManifest(null, { version: 1, sections: [] }));
...
function cmdAdd(args) {
  ...
  const { path, manifest, raw, violations } = loadManifest(atlasDir);
  ...
  try {
    ...
    writeFileSync(path, serializeManifest(raw, manifest));
  } catch (e) {
...
function cmdCheck(args) {
  ...
  // Advisory, never a gate: it reports a prose habit, not a trust failure, so it leaves both
  // `--gate` and `--claims-gate` exactly as they were.
  let uncited = 0;
  const reportComparisons = (s) => {
    for (const sentence of uncitedComparisons(atlasDir, s)) {
      uncited++;
      console.log(`      !! uncited comparison: ${sentence}`);
    }
  };
  ...
  const uncitedCell = uncited ? `, ${uncited} uncited comparison(s)` : '';
  console.log(`\n${manifest.sections.length} section(s), ${stale} stale, ${unmappedCell}${uncitedCell}.`);
...
function cmdStamp(args) {
  ...
  try { writeFileSync(path, serializeManifest(raw, manifest)); }
