---
name: code-ops-suite-data-model
...
**Invoked as `/code-ops-suite-data-model`, or by the model through the `skill` tool as `code-ops-suite-data-model`.** First read the `<plugin-root>/CONVENTIONS.md` — especially the **documentation quality standard (`§13`)**. For this DOCUMENT-mode skill the binding sections are §2 (tools/in-house docs lookup), §3 (interaction), §4 (safety rails), §12 (SSOT/registers), and §13 (doc standard) — read those five; the fan-out/fix machinery (§1, §5–§8, §11) does not apply here.
