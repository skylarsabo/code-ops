...
**Invoked as `/researcher-library-eval`, or by the model through the `skill` tool as `researcher-library-eval`.** First read the bundled `<plugin-root>/CONVENTIONS.md` — the research-integrity & egress model (`§A`), protocol, rails, evidence/citation discipline, hand-off map, and lenses, referenced by section.
