---
name: code-ops-suite-local-review-gate
...
**Invoked as `/code-ops-suite-local-review-gate`, or by the model through the `skill` tool as `code-ops-suite-local-review-gate`.** First read the `<plugin-root>/CONVENTIONS.md` bundled with this plugin — operating model (`§1`), evidence and refutation (`§7`), artifact safety (`§9`), quality lenses (`§10`), and run ledgers (`§12`) govern this skill.
