---
name: code-ops-suite-doc-alignment
...
**Invoked as `/code-ops-suite-doc-alignment`, or by the model through the `skill` tool as `code-ops-suite-doc-alignment`.** First read the `<plugin-root>/CONVENTIONS.md` bundled with this plugin (search the plugin directory for it if needed) — it defines the operating model, interaction protocol, safety rails, schemas, and quality lenses this skill references by section.
