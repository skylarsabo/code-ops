...
**Invoked as `/code-ops-suite-adr`, or by the model through the `skill` tool as `code-ops-suite-adr`.** First read the `<plugin-root>/CONVENTIONS.md` — especially the **documentation quality standard (`§13`)**.
**Mode:** DOCUMENT. **Produces:** numbered ADRs in `code-ops-docs/20 Decisions/ADRs/` (match an existing ADR convention if one is present).
...
## Assemble (per `§13`)
Maintain a `code-ops-docs/20 Decisions/ADRs/README.md` index. For backfill, rank by how load-bearing the decision is and surface the riskiest / most-questioned first. Mark reconstructed rationale `UNVERIFIED` where the history is silent.
