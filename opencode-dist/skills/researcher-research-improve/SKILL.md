...
**Invoked as `/researcher-research-improve`, or by the model through the `skill` tool as `researcher-research-improve`.** First read the bundled `<plugin-root>/CONVENTIONS.md` — the research-integrity & egress model (`§A`), protocol, rails, schemas, tiers, and lenses, referenced by section.
