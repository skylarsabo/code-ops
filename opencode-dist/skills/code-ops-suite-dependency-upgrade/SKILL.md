---
name: code-ops-suite-dependency-upgrade
...
**Invoked as `/code-ops-suite-dependency-upgrade`, or by the model through the `skill` tool as `code-ops-suite-dependency-upgrade`.** First read the `<plugin-root>/CONVENTIONS.md` bundled with this plugin (search the plugin directory for it if needed) — it defines the operating model, interaction protocol, safety rails, schemas, and quality lenses this skill references by section.
