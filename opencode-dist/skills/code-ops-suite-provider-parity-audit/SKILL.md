---
name: code-ops-suite-provider-parity-audit
...
**Invoked as `/code-ops-suite-provider-parity-audit`, or by the model through the `skill` tool as `code-ops-suite-provider-parity-audit`.** First read the
`<plugin-root>/CONVENTIONS.md` bundled with this plugin, and especially the finding and
...
- **Intentionally provider-specific.** The prose is deliberately Claude-only, for example a opencode hook feature with no Codex equivalent, and documents why inline. Leave it as it stands and record the rationale.
...
- `FINDINGS_REGISTER.md` passes `node <plugin-root>/scripts/revalidate-register.mjs FINDINGS_REGISTER.md --root .` clean.
