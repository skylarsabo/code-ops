...
**Invoked as `/code-ops-suite-provider-parity-audit`, or by the model through the `skill` tool as `code-ops-suite-provider-parity-audit`.** First read the `<plugin-root>/CONVENTIONS.md` bundled with this plugin — the finding/fix tracks (`§6`) and finding schema (`§7`) this skill's findings use.
...
- **Intentionally provider-specific** — the prose is deliberately Claude-only (e.g. a opencode hook feature with no Codex equivalent) and documents why inline; leave as-is but record the rationale.
