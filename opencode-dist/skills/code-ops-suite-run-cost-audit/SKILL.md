...
**Invoked as `/code-ops-suite-run-cost-audit`, or by the model through the `skill` tool as `code-ops-suite-run-cost-audit`.** First read the `<plugin-root>/CONVENTIONS.md` bundled with this plugin — specifically the bounded-wave fan-out rule and model/effort routing (`§1`) and the artifact-size length discipline (`§12`) this skill audits against.
...
- **Artifact-size bounds (`§12`).** Run `node <plugin-root>/scripts/co.mjs scan narration <run folder>/EXECUTIVE_SUMMARY.md <other run summaries>` — a HARD hit is an over-length or narrated artifact, an advisory is a borderline one.
