...
**Invoked as `/code-ops-suite-repo-docs`, or by the model through the `skill` tool as `code-ops-suite-repo-docs`.** First read `<plugin-root>/CONVENTIONS.md` and its orchestration, evidence, and documentation sections.
