...
A portable toolkit of operational prompts for running serious engineering work on **any codebase**, in any language, with a capable agentic coding agent (e.g. opencode). Bundled with the **code-ops-suite** plugin. Each skill in this plugin is invoked as a namespaced slash command (e.g. `/code-ops-suite-codebase-audit`) and runs an adaptive, multi-agent workflow to completion.
...
- **Skim huge files, then deepen.** For a very large file, skim first (structure, exports/signatures, the risky regions) and deepen on what matters, rather than reading it end-to-end. `<plugin-root>/scripts/skim.mjs <file>` prints the outline with line ranges; read a range, not the file. `<plugin-root>/scripts/context-query.mjs find|callers|callees|blast <symbol or path>` answers a structural question with anchors, so search the index before you read a whole map.
