...
`hooks/dispatch-guard.mjs` is the mechanical backstop for all four rules, because the Round budget was advisory and did not bind: one operative spent 71 tool uses against a 40-round budget. Inside a subagent the hook counts tool calls, tells the operative at the budget to checkpoint and return, repeats that every further 20 rounds, and denies further tool calls at three times the budget. On the lead's own dispatch it never denies; it adds one short advisory naming a model override, a wide-surface or context-inheriting agent type, or a brief with no Round budget. `CODE_OPS_DISPATCH_GUARD` takes `off` to disable it and `warn` to keep the advisories without the hard stop, and `CODE_OPS_ROUND_BUDGET` moves the default of 40.
