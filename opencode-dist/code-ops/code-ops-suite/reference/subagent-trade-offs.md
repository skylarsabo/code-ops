...
`hooks/dispatch-guard.mjs` is the mechanical backstop for all four rules, because the Round budget was advisory and did not bind: one operative spent 71 tool uses against a 40-round budget. Inside a subagent the hook counts tool calls, tells the operative at the budget to checkpoint and return, repeats that every further 20 rounds, and denies further tool calls at twice the budget. Reviewers averaged about 90 rounds in the 10-day audit, under the old stop at three times the budget, so that stop never bound them. On the lead's own dispatch it denies a `general-purpose`, `claude`, `fork`, or unnamed agent type unless the brief carries a `Wide-surface reason:` line, because general-purpose operatives carried 29% of that audit's input tokens. It also denies a new dispatch once the lead's context passes the ceiling, 300,000 tokens by default, until `/code-ops-suite-handoff assess` runs. Otherwise it adds one short advisory naming a model override or a brief with no Round budget. `CODE_OPS_DISPATCH_GUARD` takes `off` to disable it and `warn` to turn each deny into an advisory, `CODE_OPS_ROUND_BUDGET` moves the default of 40, and `CODE_OPS_CONTEXT_CEILING` moves or disables the ceiling.

Every agent definition also carries a `Report cap: at most N words` line: 600 for `reviewer`, `privacy-reviewer`, and `implementer`, and 400 for every other agent. A writing agent's cap binds the report it returns inline. The report re-enters the lead's context and is re-read on every later turn. Lint check 25 fails an agent with no cap or a cap outside 100 to 800 words. A brief may name a smaller cap.
