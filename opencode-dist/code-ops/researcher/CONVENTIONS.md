...
- **Task-based routing.** Every new non-calibration substantive version 4 run requires `routingPolicy: "task-based"`; legacy replay packets and calibration arms retain their existing rules. Select role, model tier, and effort from the unit's work and ambiguity. Keep every declared floor. A frontier peer needs a bounded class, rationale, stopping criterion, and a lead-owned blocking criterion attached to its unit. One peer is the limit.
