...
**Invoked as `/researcher:ecosystem-watch`.** First read the bundled
...
types through `${CLAUDE_PLUGIN_ROOT}/scripts/lib-docs.mjs`, or through the `code-ops-docs`
...
`node ${CLAUDE_PLUGIN_ROOT}/scripts/revalidate-register.mjs ECOSYSTEM_WATCH.md --root <repo>`
and
`node ${CLAUDE_PLUGIN_ROOT}/scripts/research-manifest.mjs validate ECOSYSTEM_WATCH.md`, which
