...
**Invoked as `/researcher:research-improve`.** Read §A, §2, §3, §4, §6, §7, §8, §10, §11, §12, and §14 of
...
`${CLAUDE_PLUGIN_ROOT}/scripts/lib-docs.mjs` (`§2`), and anything the developer hands you.
...
- **Opt-in web, only if it was granted in Phase 0.** Compose the `deep-research` skill,
  ...
  request with `node ${CLAUDE_PLUGIN_ROOT}/scripts/research-manifest.mjs record ...`, storing
...
`node ${CLAUDE_PLUGIN_ROOT}/scripts/revalidate-register.mjs RESEARCH_FINDINGS.md --root .`,
...
`node ${CLAUDE_PLUGIN_ROOT}/scripts/research-manifest.mjs validate RESEARCH_FINDINGS.md`,
