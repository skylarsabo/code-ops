...
- **Dependency capabilities** — what installed libraries already offer that we don't use: `${CLAUDE_PLUGIN_ROOT}/scripts/lib-docs.mjs` (or the `code-ops-docs` MCP when `code-ops-suite` is installed) — reads the **installed** version, zero query egress (`§2`).
