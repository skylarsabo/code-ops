...
- **Dependency capabilities.** Dispatch gatherers, in parallel over disjoint dependencies,
  ...
  `${CLAUDE_PLUGIN_ROOT}/scripts/lib-docs.mjs`, or the `code-ops-docs` MCP server when
...
`node ${CLAUDE_PLUGIN_ROOT}/scripts/research-manifest.mjs validate IDEAS_REGISTER.md`
...
pre-filtered with
`node ${CLAUDE_PLUGIN_ROOT}/scripts/revalidate-register.mjs IDEAS_REGISTER.md --root <repo>`
