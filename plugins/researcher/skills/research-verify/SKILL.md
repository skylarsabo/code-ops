...
**Invoked as `/researcher:research-verify`.** Read only the sections named in this paragraph of
...
installed version through `${CLAUDE_PLUGIN_ROOT}/scripts/lib-docs.mjs`, or through the
...
`node ${CLAUDE_PLUGIN_ROOT}/scripts/research-manifest.mjs record ...`, storing the time, the
