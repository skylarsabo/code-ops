...
**Invoked as `/privacy-opsec-suite:authorship-hygiene`.** Read §A, §3, §4, §9, §12, and §13 of
...
`node ${CLAUDE_PLUGIN_ROOT}/scripts/co.mjs scan ai-tells <commit-range-or-pr-body-file>`. It
...
also passes `node ${CLAUDE_PLUGIN_ROOT}/scripts/co.mjs scan redaction` with no fail-closed
