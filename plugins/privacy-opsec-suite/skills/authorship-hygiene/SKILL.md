...
Strip attribution + tool markers: `Co-Authored-By:` tool trailers, "Generated with/by <tool>", AI markers in branch names, bot author/committer identities; set author/committer to the human's git identity. The mechanical floor is `node ${CLAUDE_PLUGIN_ROOT}/scripts/scan-ai-tells.mjs <commit-range-or-pr-body-file>` — it flags trailers, tool markers, emoji, em-dash density, assistant-prose tells, and the `## Test plan` boilerplate. When an operator intentionally scans one edited prose artifact, `--emdash-baseline-file <pre-edit-file>` measures net dash-count growth against that verified file. The baseline must resolve to a different file than the scan target. The option requires exactly one file target and cannot be combined with `--git`. It never suppresses the other checks.
