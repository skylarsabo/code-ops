...
Strip attribution + tool markers: `Co-Authored-By:` tool trailers, "Generated with/by <tool>", AI markers in branch names, bot author/committer identities; set author/committer to the human's git identity. The mechanical floor is `node ${CLAUDE_PLUGIN_ROOT}/scripts/co.mjs scan ai-tells <commit-range-or-pr-body-file>` — it flags trailers, tool markers, emoji, em-dash density, assistant-prose tells, and the `## Test plan` boilerplate. When an operator intentionally scans one edited tracked artifact, `--emdash-baseline-rev <pre-edit-revision>` measures dash-count growth against that target's blob at an ancestor of `HEAD`. Arbitrary baseline files are not accepted. The option requires exactly one file target and cannot be combined with `--git`. It never suppresses the other checks.
