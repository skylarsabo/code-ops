...
Run `node ${CLAUDE_PLUGIN_ROOT}/scripts/preflight.mjs --artifact-dir <run folder>` — a FAIL stops the run before any fan-out; advisories are noted in the register. After preflight passes, run `node ${CLAUDE_PLUGIN_ROOT}/scripts/repo-map.mjs --out <run folder>/REPO_MAP.md`; on failure, note the advisory and proceed. Dispatch the explorer subagent to detect the stack and the size of the repo, handing its summary plus `REPO_MAP.md` to the next phase, then confirm with me:
