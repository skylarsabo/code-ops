...
- **Background dispatch.** Dispatch operatives in the background and continue independent work. Wait only when the next step depends on the result. The dispatch ledger records the overlap so the wall-time gain is measured.
