...
- **Skim huge files, then deepen.** For a very large file, skim first (structure, exports/signatures, the risky regions) and deepen on what matters, rather than reading it end-to-end. `${CLAUDE_PLUGIN_ROOT}/scripts/skim.mjs <file>` prints the outline with line ranges; read a range, not the file.
