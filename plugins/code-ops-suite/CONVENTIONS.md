...
- **Skim huge files, then deepen.** For a very large file, skim first (structure, exports/signatures, the risky regions) and deepen on what matters, rather than reading it end-to-end. `${CLAUDE_PLUGIN_ROOT}/scripts/skim.mjs <file>` prints the outline with line ranges; read a range, not the file. `${CLAUDE_PLUGIN_ROOT}/scripts/context-query.mjs find|callers|callees|blast <symbol or path>` answers a structural question with anchors, so search the index before you read a whole map.
...
- **Index once, bundle by scope.** At Phase 0 prepare an exact `CONTEXT_SNAPSHOT.json` with `${CLAUDE_PLUGIN_ROOT}/scripts/context-snapshot.mjs`; its content-addressed cache reuses an unchanged repo map and import graph across runs. Compile one verified bundle per unit; **hand the verified context artifact to every operative brief; operatives consult it first and use search only to go deeper than it reaches, never to re-derive layout or find definitions it already lists.** Context drift, broad-context markers, and byte-budget markers are replan triggers, never advisories to ignore.
...
- **Compile substantive runs before fan-out.** Orchestrators write `RUN_CONTRACT.json` after Phase 0 and validate it with `${CLAUDE_PLUGIN_ROOT}/scripts/run-contract.mjs check`. Use version 2 for bounded runs. Use version 3 for a multi-phase or resumable run. A version 3 contract declares host capabilities, runtime policy, receipt path, and a bounded stable prefix. Initialize `HOST_CAPABILITIES.json` from observed host facts. Do not infer capabilities from a model name. Invoke `run-runtime.mjs init` before fan-out. At every phase boundary, reconcile `DISPATCH_LEDGER.md`, checkpoint the referenced artifacts, then replan or resume only through the receipt chain. Increment `revision` when learning, context, or runtime binding changes. Never rewrite the plan merely to match an unplanned dispatch.
...
4. **Implement** — match existing conventions; uphold the relevant lenses (§10); don't trade one issue for a new one. Edit surgically. Rewrite a whole file only when the file is short or most of it changes. A pre-existing bug, performance concern, or behavior the task does not name is reported as a follow-up, never fixed, optimized, or extended in this change unless the requested behavior cannot work without it.
5. **Test** — add/adjust tests that fail before and pass after; cover edge/error paths. Commit tests only where the task asks for them or the repository already keeps tests for this kind of change, sized like the neighboring test files, and never turn scratch checks into permanent test files.
...
- **Standard filenames** prompts produce: `FINDINGS_REGISTER.md`, `FEATURE_OPPORTUNITIES.md` + feature specs, `EXECUTIVE_SUMMARY.md`, `REFUTATION_LOG.md`, `REPO_MAP.md`, `RUN_CONTRACT.json`, `DISPATCH_LEDGER.md`, `ACCEPTANCE_LEDGER.md`, `RUN_CONTRACT_RESULT.json`, and per-prompt logs/reports named in each prompt. A version 3 run also owns `HOST_CAPABILITIES.json` and `RUN_RUNTIME_RECEIPTS.jsonl`. The receipt chain replaces `RUN_RECEIPTS.md` for runtime state.
