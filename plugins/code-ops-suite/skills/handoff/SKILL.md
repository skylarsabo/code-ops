...
`node ${CLAUDE_PLUGIN_ROOT}/scripts/co.mjs run open <slug> --name "<program name>"`. One run folder
...
resolve the live head with `node ${CLAUDE_PLUGIN_ROOT}/scripts/co.mjs handoff live "<name>"`: a
...
`node ${CLAUDE_PLUGIN_ROOT}/scripts/co.mjs handoff resume <HANDOFF.md or session name> --root .` as
