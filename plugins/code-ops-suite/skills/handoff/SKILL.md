...
`${CLAUDE_PLUGIN_ROOT}/CONVENTIONS.md` bundled with this plugin. Those four sections are the
...
`node ${CLAUDE_PLUGIN_ROOT}/scripts/run-runtime.mjs status --root . --contract <contract>`.
...
`node ${CLAUDE_PLUGIN_ROOT}/scripts/co.mjs check handoff HANDOFF.md --consume`. That writes
