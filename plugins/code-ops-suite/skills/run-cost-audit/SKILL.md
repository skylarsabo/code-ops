...
This audit is the **data producer** for the forward-looking half of the same loop: every stamped ledger it reads back is what `node ${CLAUDE_PLUGIN_ROOT}/scripts/estimate-run-cost.mjs --runs <runs dir> --skill <name>` uses at the NEXT run's Phase 0 to estimate a dispatch-count range and model-class mix before the budget is spent (see `docs/handbook/09-cost-and-scoping.md`).
