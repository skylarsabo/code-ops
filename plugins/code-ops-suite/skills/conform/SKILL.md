...
**Invoked as `/code-ops-suite:conform`.** First read the `${CLAUDE_PLUGIN_ROOT}/CONVENTIONS.md`
...
| 3 | Atlas | `code-ops-docs/98 System/Atlas/` exists (fallback `atlas/`), its manifest parses, and `node ${CLAUDE_PLUGIN_ROOT}/scripts/atlas-check.mjs check --atlas <atlas dir>` reports each section FRESH or STALE | `/code-ops-suite:atlas` |
