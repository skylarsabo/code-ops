...
- **Backfill** — mine the code, VCS history, and existing docs for the **load-bearing decisions already made** (the "why is it this way?" choices a newcomer hits). Write an ADR for each, grounded in evidence.
...
## Done when
Each ADR states a real decision with its true context, the options actually weighed, and honest consequences — traceable to the code that implements it; the index lists them; a senior engineer understands *why* the system is the way it is, not just what it is.
