...
**Invoked as `/code-ops-suite:provider-parity-audit`.** First read the `${CLAUDE_PLUGIN_ROOT}/CONVENTIONS.md` bundled with this plugin — the finding/fix tracks (`§6`) and finding schema (`§7`) this skill's findings use.
...
- **Intentionally provider-specific** — the prose is deliberately Claude-only (e.g. a Claude Code hook feature with no Codex equivalent) and documents why inline; leave as-is but record the rationale.
...
## Done when
Every hit from Phase 0 has a classification; `FINDINGS_REGISTER.md` passes `node ${CLAUDE_PLUGIN_ROOT}/scripts/revalidate-register.mjs FINDINGS_REGISTER.md --root .` clean. Present the register, needs-rewording items first.
