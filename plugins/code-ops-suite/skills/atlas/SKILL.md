...
**The tool owns the manifest.** `${CLAUDE_PLUGIN_ROOT}/scripts/atlas-check.mjs` writes every part of it — `add` registers a section, `stamp` is the only sanctioned writer of `verifiedAt`. `MANIFEST.json` is never hand-edited; that is how a section that has gone stale keeps claiming to be current.
