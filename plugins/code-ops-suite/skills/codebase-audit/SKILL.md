...
**Invoked as `/code-ops-suite:codebase-audit`.** First read §1, §3, §4, §6, §7, §9, §10, §11, §12, and §14 of the
...
`node ${CLAUDE_PLUGIN_ROOT}/scripts/co.mjs scan overbuild --git <range>`, which reports eight
...
- The finished `FINDINGS_REGISTER.md` passes `node ${CLAUDE_PLUGIN_ROOT}/scripts/revalidate-register.mjs FINDINGS_REGISTER.md --root .` with exit 0. Re-locate a non-FRESH citation against the real tree, or drop it, before the run is done (`§9`).
