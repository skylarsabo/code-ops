...
`node ${CLAUDE_PLUGIN_ROOT}/scripts/revalidate-register.mjs FINDINGS_REGISTER.md --root .` and
...
not a fix. Run `node ${CLAUDE_PLUGIN_ROOT}/scripts/co.mjs scan overbuild --git <range>` over the
