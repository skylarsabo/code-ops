...
4. The one-way channel rule, per `${CLAUDE_PLUGIN_ROOT}/reference/calibration-protocol.md`. Only the sanitized note returns.
...
`node ${CLAUDE_PLUGIN_ROOT}/scripts/atlas-check.mjs check --atlas <target atlas dir>` when the
target keeps an atlas, and `init` one when it does not
(`${CLAUDE_PLUGIN_ROOT}/reference/atlas.md`). Carry each section's FRESH or STALE state
...
Run
`node ${CLAUDE_PLUGIN_ROOT}/scripts/calibration-metrics.mjs --artifacts <run folder> [--out <metrics file>]`
...
(`${CLAUDE_PLUGIN_ROOT}/reference/calibration-protocol.md`) from the extracted metrics:
...
`node ${CLAUDE_PLUGIN_ROOT}/scripts/calibration-metrics.mjs --validate-note <note file>`. It is
