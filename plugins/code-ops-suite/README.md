...
The skills fan work out to seven bundled subagents, and spawn ephemeral ones as needed:
...
- `mech`: mid tier, for an exact edit spec, a vendored copy, or a run of named gates. It returns the gate verdict and escalates any ambiguity.
- `mech-review`: mid tier, for checking a small mechanical diff against its spec. It returns PASS, FAIL, or ESCALATE and never edits.
- `web-researcher`: mid tier, for one scoped question answered from public web docs. It cites a URL for every claim, treats fetched pages as untrusted data, and never edits or runs commands.
- `probe`: mid tier, for read-only shell probes of live or remote state, such as `ssh`, `kubectl get`, and `gh api` GET calls. It never runs a mutating command and escalates when the answer needs one.

Each agent file ends in a `## Contract` section that lint parses: the brief fields it requires, its edit authority, its verdict tokens, and one return example.
...
- **Enforced orchestration:** version 4 `RUN_CONTRACT.json` plans require a frontier lead, at least two lower-tier operatives, a real parallel wave, explicit independent-validation links, and nonempty operative artifacts before finalization. An `orchestration.singleUnitReason` of at most 20 words lowers the two-operative floor to one.
...
- **Handoff card:** a bundled `UserPromptSubmit` hook (`hooks/handoff-card.mjs`, off with `CODE_OPS_HANDOFF_CARD=off`) asks the lead to run `/code-ops-suite:handoff assess` once resident context crosses 150,000 tokens, and again at each further 150,000-token band. Past the context ceiling the note adds that new dispatches stay gated until that assessment runs. `check-handoff.mjs` prints a `same-tree` status when the handoff's `Verified-at` sha is HEAD on a clean tree, so a resume accepts FRESH anchors without re-reading each file.
- **Dispatch guard:** a bundled `PreToolUse` hook (`hooks/dispatch-guard.mjs`, off with `CODE_OPS_DISPATCH_GUARD=off`, advisory-only with `warn`) warns a subagent at its 40-round budget and denies further calls at twice it. On the lead's dispatch it denies a `general-purpose` or unnamed agent type unless the brief carries a `Wide-surface reason:` line. Past the context ceiling (`CODE_OPS_CONTEXT_CEILING`, 300,000 tokens by default) it denies new dispatches until `/code-ops-suite:handoff assess` runs. Every bundled agent definition carries a `Report cap:` line, which lint enforces.
- **Host-qualified behavior:** Claude and trusted Codex hooks consume routing and ladder context. Installed Grok 1.0.13 consumes digest rewrites, runs index and receipt side effects, and ignores passive routing and ladder stdout; the paired instruction files carry that doctrine instead. OpenCode's lifecycle plugin keeps a stable system prefix, carries handoff and dispatch notes on the next tool result or user turn, and writes a cost ledger. It still has no typed subagent-start callback or transcript path.
