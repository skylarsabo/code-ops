...
Run `node ${CLAUDE_PLUGIN_ROOT}/scripts/revalidate-register.mjs FINDINGS_REGISTER.md --root .` as a fast staleness pre-filter (`CONVENTIONS §10`), then for each CONFIRMED finding confirm its **repro still fails** on current code (drop or re-tier anything that no longer reproduces). Build a dependency/conflict graph; sequence by demonstrated impact (`§D`). For **NEEDS-DESIGN** items, present options and get a direction first.
